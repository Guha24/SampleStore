...
function searchPage(productSearch){
  let productsHTML='';
  ...
  productsHTML+=`<a href="index.html" class="back-link">Back to Home --> </a>`
  ...
  document.querySelector('.js-search-button')
    .addEventListener('click',()=>{
      ...
      products.forEach((productItem)=>{
        const keywords=productItem.keywords;
        const name=productItem.name;
        let flag=0;
        keywords.forEach((keyItem)=>{
          if(keyItem===input){
            productSearch.push(productItem);
            flag=1;
          }
        });
        if(flag === 0){
